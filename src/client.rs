...
use crate::protocol::{
    ConnectionState, HandshakePacket, StatusRequest, StatusResponse, 
    PingRequest, PongResponse, packet::parse_packet_data, Packet
};
...
impl ClientConnection {
    ...
    async fn process_data(&mut self, data: &[u8]) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let mut offset = 0;
        ...
        while offset < data.len() {
            ...
            let (packet_length, length_size) = match crate::protocol::varint::VarInt::from_bytes(remaining) {
            ...
            if remaining.len() < length_size + packet_length {
                tracing::debug!("Incomplete packet from {} (need {} bytes, have {})", 
                    self.addr, length_size + packet_length, remaining.len());
                break;
            }
            ...
            tracing::debug!("Processing packet from {} (length: {}, data: {:?})", 
                self.addr, packet_length, packet_data);
    ...
    async fn handle_packet(&mut self, packet_data: &[u8]) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let (packet_id, data) = parse_packet_data(packet_data)?;
        
        tracing::debug!("Handling packet ID {} in state {:?} from {}", 
            packet_id, self.state, self.addr);
        ...
        match self.state {
            ConnectionState::Handshaking => {
                if packet_id == HandshakePacket::packet_id() {
                    ...
                    tracing::warn!("Unexpected packet {} in handshaking state from {}", 
                        packet_id, self.addr);
                }
            }
            ConnectionState::Status => {
                match packet_id {
                    ...
                    _ => {
                        tracing::warn!("Unexpected packet {} in status state from {}", 
                            packet_id, self.addr);
                    }
                }
            }
            _ => {
                tracing::warn!("Unhandled state {:?} for packet {} from {}", 
                    self.state, packet_id, self.addr);
            }
        }
    ...
    async fn handle_handshake(&mut self, data: &[u8]) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Some(next_state) = handshake.get_next_state() {
            ...
            tracing::debug!("Client {} transitioned to state {:?}", self.addr, self.state);
        } else {
            tracing::warn!("Invalid next_state {} in handshake from {}", 
                handshake.next_state, self.addr);
        }
    ...
    async fn handle_status_request(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    /// Handle ping request
    async fn handle_ping_request(&mut self, data: &[u8]) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let mut cursor = Cursor::new(data);
        let ping = PingRequest::read_data(&mut cursor)?;
        
        tracing::debug!("Ping request from {} with payload {}", self.addr, ping.payload);
    ...
    async fn send_packet<P: Packet>(&mut self, packet: &P) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        use tokio::io::AsyncWriteExt;
        
        let mut buffer = Vec::new();
        packet.write_packet(&mut buffer)?;
        
        tracing::debug!("Sending packet to {} (length: {}, data: {:?})", 
            self.addr, buffer.len(), buffer);
