...
use flate2::{Compress, Compression as FlateCompression, Decompress, FlushCompress, FlushDecompress, Status};
...
impl Compression {
    ...
    pub fn compress_packet(&mut self, packet_id: VarInt, data: &[u8]) -> Result<Vec<u8>> {
        ...
        loop {
            ...
            let status = self.compressor.compress(
                ...
                if input_pos == uncompressed_length { FlushCompress::Finish } else { FlushCompress::None }
            )?;
    ...
    pub fn decompress_packet(&mut self, data: &[u8]) -> Result<(VarInt, Vec<u8>)> {
        ...
        if data_length.0 == 0 {
            ...
            let remaining_data = compressed_data[uncompressed_cursor.position() as usize..].to_vec();
        ...
        loop {
            ...
            match status {
                ...
                Status::StreamEnd => break,
                Status::BufError => {
                    return Err(ServerError::Protocol("Decompression buffer error".to_string()));
                }
            }
        }
        ...
        if output_pos != uncompressed_length {
            return Err(ServerError::Protocol(
                format!("Decompressed length mismatch: expected {}, got {}", 
...
mod tests {
    ...
    fn test_compression_roundtrip() {
        ...
        let large_data = vec![0x00; 1000]; // Large data that will be compressed
        
        // Test small packet (uncompressed)
        ...
        // Test large packet (compressed)
        let compressed = compression.compress_packet(packet_id, &large_data).unwrap();
        let (decoded_id, decoded_data) = compression.decompress_packet(&compressed).unwrap();
        
        assert_eq!(packet_id, decoded_id);
        assert_eq!(large_data, decoded_data);
    }
}
